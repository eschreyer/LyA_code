...
we cut on the sound speed so that the gas cannot go sonic outside hill sphere or before planet radius
...
sampled_parameters = ['c_s_planetb', 'mdot_planetb', 'c_s_planetc', 'mdot_planetc', 'v_stellar_wind', 'mdot_star', 'L_EUV', 'angleb', 'anglec', 'inclinationb', 'inclinationc']
sampled_parameter_guess = np.array([5.7, 9, 5.7, 8.8, 7.2, 12, 28.3, (3/5)*np.pi, (3/5)*np.pi, 1.565, 1.563])
...
def evaluate_log_priorb(lp, constant_parameters):
    ...
    hill_sphere_radius = constant_parameters['semimajoraxis']*(constant_parameters['mass_p']/(3*constant_parameters['mass_s']))**(1/3)

    #uniform(and log uniform priors)
    if np.log10(np.sqrt(const.G * constant_parameters['mass_p'] / (2 * hill_sphere_radius))) <= lp['c_s_planet'] <= np.log10(np.sqrt(const.G * constant_parameters['mass_p'] / (2 * constant_parameters['radius_p'])))\
    and 8 <= lp['mdot_planet'] <= np.log10(energy_limited_mlr) + 1\
